...
interface InvestmentRoadmapSliderProps {
  missions: InvestmentMission[];
  onMissionComplete?: (missionId: string, files: FileList) => Promise<void>;
  className?: string;
}

export const InvestmentRoadmapSlider: React.FC<InvestmentRoadmapSliderProps> = ({
  missions,
  onMissionComplete,
  className = ''
}) => {
  ...
  const [selectedMission, setSelectedMission] = useState<InvestmentMission | null>(null);
  ...
  // 미션 시작하기
  const handleStartMission = (mission: InvestmentMission) => {
  ...
  // 미션 카드 컴포넌트
  const MissionCard = ({ mission }: { mission: InvestmentMission }) => (
    <div className="relative">
      ...
      <div className="flex justify-center mb-1 overflow-hidden h-20">
        <div className={`w-12 h-12 transform rotate-45 flex items-center justify-center relative ${
          mission.status === 'completed'
            ...
            ? 'bg-gradient-to-br from-indigo-400 to-indigo-600 shadow-md animate-pulse'
      ...
      <div className={`p-4 rounded-lg border-2 h-40 flex flex-col justify-between ${
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          {mission.status === 'completed' && (
            <span className="text-purple-600 text-sm font-medium">완료됨</span>
          )}
        </div>
      </div>
    </div>
