...
export const RoadmapSlider: React.FC<RoadmapSliderProps> = ({
  missions,
  onMissionComplete,
  className = ''
}) => {
  ...
  // 미션 카드 컴포넌트
  const MissionCard = ({ mission }: { mission: RoadmapMission }) => (
    <div className="relative">
      ...
      <div className={`p-4 rounded-lg border-2 min-h-[10rem] sm:h-40 flex flex-col justify-between ${
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          {mission.status === 'completed' && (
            <span className="text-yellow-600 text-sm font-medium">완료됨</span>
          )}
        </div>
      </div>
    </div>
    ...
    <div className={`${className}`}>
      ...
      <div className="relative bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 space-y-4 sm:space-y-0">
          <h3 className="hidden sm:block text-lg font-semibold text-gray-900">성장 로드맵</h3>
